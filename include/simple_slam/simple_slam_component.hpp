...
#pragma omp parallel for schedule(dynamic)
  ...
  class SimpleSlam : public ext_rclcpp::ExtensionNode {
  public:
    SimpleSlam(const rclcpp::NodeOptions& options) : SimpleSlam("", options) {}
    SimpleSlam(const std::string& name_space = "", const rclcpp::NodeOptions& options = rclcpp::NodeOptions())
        : ext_rclcpp::ExtensionNode("simple_slam_node", name_space, options), tf_buffer_(this->get_clock()), listener_(tf_buffer_) {
      RCLCPP_INFO(this->get_logger(), "start simple_slam_node");
      // setup
      SMAP_PUBLISH_RATE = param<double>("simple_slam.storage_map.publish_rate", 1.0);
      VOXELGRID_SIZE    = param<double>("simple_slam.voxel_grid.size", 0.1);
      kill_switch_      = false;
      // logger
      // const int ringbuffer_size = 100;
      // ringbuffer_sink_          = std::make_shared<spdlog::sinks::ringbuffer_sink_mt>(ringbuffer_size);
      logger_ = spdlog::get("simple_slam_logger");
      // logger_->set_level(spdlog::level::trace);
      // logger_->sinks().emplace_back(ringbuffer_sink_);
      // init
      // publisher
      out_cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("simple_slam/out_points", rclcpp::QoS(10));
      // subscriber
...
RCLCPP_COMPONENTS_REGISTER_NODE(simple_slam::SimpleSlam)
